...
---------------------------
Most NTU courses are named ``26S1-SC2005-OPERATING SYSTEMS``, but not all. Observed live
in one student's enrolment list:
    ...
    Personal Data Protection Act (PDPA) e-Learning      no semester at all

Matching only ``26S1-`` would silently drop a real current-semester course, which is the
worst possible failure for a sync tool: it looks like it worked.

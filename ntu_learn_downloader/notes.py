...
class Notebook:
    ...
    @staticmethod
    def _summary_row(row) -> Optional[Dict]:
        if not row:
            return None
        return dict(zip(
            ("id", "document_id", "material_name", "prompt", "scope", "backend",
             "model", "rung", "tex_path", "pdf_path", "page_anchors", "ok", "error",
             "created_at"),
            row))
    ...
    def save_summary(self, summary_id: str, document_id: str, material_name: str = "",
        ...
        with self._lock, self._connect() as db:
            db.execute("""INSERT INTO summaries(id, document_id, material_name,
                ...
                page_anchors, ok, error, created_at)
                VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?)
    ...
    def get_summary(self, summary_id: str) -> Optional[Dict]:
        with self._lock, self._connect() as db:
            row = db.execute("""SELECT id, document_id, material_name, prompt,
                scope, backend, model, rung, tex_path, pdf_path, page_anchors, ok,
                error, created_at FROM summaries WHERE id=?""",
                (str(summary_id)[:64],)).fetchone()
        return self._summary_row(row)

    def list_summaries(self, document_id: str, limit: int = 20) -> List[Dict]:
        with self._lock, self._connect() as db:
            rows = db.execute("""SELECT id, document_id, material_name, prompt,
                scope, backend, model, rung, tex_path, pdf_path, page_anchors, ok,
                error, created_at FROM summaries WHERE document_id=?
    ...
    def delete_summary(self, summary_id: str) -> Optional[Dict]:
        ...
        with self._lock, self._connect() as db:
            row = db.execute("""SELECT id, document_id, material_name, prompt,
                scope, backend, model, rung, tex_path, pdf_path, page_anchors, ok,
                error, created_at FROM summaries WHERE id=?""", (summary_id,)).fetchone()
            if not row:
                return None
    ...
    def delete_summaries_older_than(self, cutoff_iso: str) -> List[Dict]:
        ...
        with self._lock, self._connect() as db:
            rows = db.execute("""SELECT id, document_id, material_name, prompt,
                scope, backend, model, rung, tex_path, pdf_path, page_anchors, ok,
                error, created_at FROM summaries WHERE created_at < ?""",
                (cutoff_iso,)).fetchall()
            if rows:

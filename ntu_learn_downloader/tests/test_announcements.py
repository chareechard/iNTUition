...
from ntu_learn_downloader.announcements import Feed, clean
...
def test_sync_aggregates_courses_and_survives_one_failure(tmp_path):
    ...
    rows = [{"id": "a", "title": "Welcome", "body": "<p>Hello</p>",
             "draft": False, "modified": "2026-08-11T09:00:00Z"}]
    ...
    with patch("ntu_learn_downloader.announcements.rest._get_paged", side_effect=paged):
...
def test_sync_retains_recent_cached_items_during_empty_response(tmp_path):
    ...
    with patch("ntu_learn_downloader.announcements.rest._get_paged", return_value=[]):
...
def test_tldr_is_cached_with_its_source_set(tmp_path):
    ...
    with patch("ntu_learn_downloader.announcements.ai_provider.complete_tier",

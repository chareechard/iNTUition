...
from intuition.api import get_contents, get_download_dir
...
class InternalTestAPI(unittest.TestCase):
    ...
    def test_rest_content_cycle_does_not_fall_back_to_legacy(self):
        with patch('intuition.rest.get_download_dir',
                   side_effect=rest.RestContentCycle('cycle')), \
             patch('intuition.api.get_download_dir_legacy') as legacy:
            with self.assertRaises(rest.RestContentCycle):
                get_download_dir('token', 'Course', 'course')

...
from intuition import get_courses, get_content_ids

from intuition.api import parse_content_page
from intuition.models import Folder, Doc, RecordedLecture
...
class TestModels(unittest.TestCase):
    def test_folder_get_children(self):
        ...
        with patch.dict("intuition.api.__dict__", MOCK_CONSTANTS):
            folder.load_children(BbRouter)
            self.assertListEqual(expected, folder.children)

    def test_folder_serialize_rejects_cyclic_content_link(self):
        link = '/webapps/blackboard/content/listContent.jsp?course_id=_1_1&content_id=_2_1'
        folder = Folder('cycle', link, '', None)
        with patch('intuition.api.make_get_contents_request', return_value=object()), \
             patch('intuition.models.parse_content_page', return_value=[object()]), \
             patch('intuition.models.to_model', return_value=folder):
            with self.assertRaisesRegex(ValueError, 'cycle'):
                folder.serialize('token')
